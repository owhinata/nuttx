/****************************************************************************
 * arch/arm/src/stm32l4/stm32l4_octospi.c
 *
...
#include "hardware/stm32l4_octospi.h"

#ifdef CONFIG_STM32L4_OCTOSPI_DMA
...
#  undef CONFIG_STM32L4_OCTOSPI_DMADEBUG
#endif
...
#if defined(CONFIG_STM32L4_OCTOSPI_INTERRUPTS) && defined(CONFIG_STM32L4_OCTOSPI_DMA)
...
#  if defined(CONFIG_STM32L4_OCTOSPI_DMAPRIORITY_LOW)
#    define QSPI_DMA_PRIO  DMA_SCR_PRILO
#  elif defined(CONFIG_STM32L4_OCTOSPI_DMAPRIORITY_MEDIUM)
#    define QSPI_DMA_PRIO  DMA_SCR_PRIMED
#  elif defined(CONFIG_STM32L4_OCTOSPI_DMAPRIORITY_HIGH)
#    define QSPI_DMA_PRIO  DMA_SCR_PRIHI
#  elif defined(CONFIG_STM32L4_OCTOSPI_DMAPRIORITY_VERYHIGH)
#    define QSPI_DMA_PRIO  DMA_SCR_PRIVERYHI
#  else
#    define QSPI_DMA_PRIO  DMA_SCR_PRIMED
#  endif

#endif /* CONFIG_STM32L4_OCTOSPI_DMA */

#ifndef STM32L4_SYSCLK_FREQUENCY
#  error your board.h needs to define the value of STM32L4_SYSCLK_FREQUENCY
#endif
...
/* On STM32L4+, OCTOSPI is clocked from HCLK (AHB clock) directly.
 * No clock source selection register is needed.
 */

#define QSPI_CLK_FREQUENCY STM32L4_HCLK_FREQUENCY

#ifndef CONFIG_STM32L4_OCTOSPI_FIFO_THRESHOLD
#  define CONFIG_STM32L4_OCTOSPI_FIFO_THRESHOLD 4
#endif

#ifndef CONFIG_STM32L4_OCTOSPI_CSHT
#  define CONFIG_STM32L4_OCTOSPI_CSHT 1
#endif
...
#ifdef CONFIG_STM32L4_OCTOSPI_INTERRUPTS
...
#ifdef CONFIG_STM32L4_OCTOSPI_DMADEBUG
  struct stm32l4_dmaregs_s dmaregs[DMA_NSAMPLES];
#endif

#ifdef CONFIG_STM32L4_OCTOSPI_REGDEBUG
...
#  ifndef CONFIG_STM32L4_OCTOSPI_DMATHRESHOLD
#    define CONFIG_STM32L4_OCTOSPI_DMATHRESHOLD 4
#  endif
...
static void qspi_dumpregs(struct stm32_qspidev_s *priv, const char *msg)
{
  ...
  spiinfo("    CR:%08x   TCR:%08x   CCR:%08x    SR:%08x\n",
          getreg32(priv->base + STM32_OCTOSPI_CR_OFFSET),     /* Control Register */
          getreg32(priv->base + STM32_OCTOSPI_TCR_OFFSET),    /* Device Configuration Register 1 */
          getreg32(priv->base + STM32_OCTOSPI_CCR_OFFSET),    /* Communication Configuration Register */
          getreg32(priv->base + STM32_OCTOSPI_SR_OFFSET));    /* Status Register */
  spiinfo("   DLR:%08x   ABR:%08x PSMKR:%08x PSMAR:%08x\n",
          getreg32(priv->base + STM32_OCTOSPI_DLR_OFFSET),    /* Data Length Register */
          getreg32(priv->base + STM32_OCTOSPI_ABR_OFFSET),    /* Alternate Bytes Register */
          getreg32(priv->base + STM32_OCTOSPI_PSMKR_OFFSET),  /* Polling Status mask Register */
          getreg32(priv->base + STM32_OCTOSPI_PSMAR_OFFSET)); /* Polling Status match Register */
  spiinfo("   PIR:%08x  LPTR:%08x DCR1:%08x DCR2:%08x\n",
          getreg32(priv->base + STM32_OCTOSPI_PIR_OFFSET),    /* Polling Interval Register */
          getreg32(priv->base + STM32_OCTOSPI_LPTR_OFFSET),   /* Low-Power Timeout Register */
          getreg32(priv->base + STM32_OCTOSPI_DCR1_OFFSET),   /* Device Configuration Register 1 */
          getreg32(priv->base + STM32_OCTOSPI_DCR2_OFFSET));  /* Device Configuration Register 2 */
  spiinfo("   DCR3:%08x  DCR4:%08x\n",
          getreg32(priv->base + STM32_OCTOSPI_DCR3_OFFSET),   /* Device Configuration Register 3 */
          getreg32(priv->base + STM32_OCTOSPI_DCR4_OFFSET));  /* Device Configuration Register 4 */
}
#endif
...
#if defined(CONFIG_STM32L4_OCTOSPI_INTERRUPTS)
...
static int qspi_waitstatusflags(struct stm32_qspidev_s *priv,
                                uint32_t mask, int polarity)
{
  uint32_t regval;
  volatile int timeout = 0x40000;

  if (polarity)
    {
      while (!((regval = qspi_getreg(priv, STM32_OCTOSPI_SR_OFFSET)) & mask))
        {
          if (--timeout <= 0)
            {
              spierr("ERROR: waitstatus timeout mask=0x%08lx\n",
                     (unsigned long)mask);
              return -ETIMEDOUT;
            }
        }
...
static void qspi_abort(struct stm32_qspidev_s *priv)
{
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
  regval |= OCTOSPI_CR_ABORT;
  qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);
}
                           ...
                           uint8_t fctn)
{
  ...
  if (CCR_DMODE_NONE != xctn->datamode && CR_FMODE_MEMMAP != fctn)
    {
      qspi_putreg(priv, xctn->datasize - 1, STM32_OCTOSPI_DLR_OFFSET);
    }
  ...
  if (xctn->altbytesmode != CCR_ABMODE_NONE)
    {
      qspi_putreg(priv, xctn->altbytes, STM32_OCTOSPI_ABR_OFFSET);
    }

  /* Build the CCR value and set it */

  regval  = OCTOSPI_CCR_IMODE(xctn->instrmode) |
            OCTOSPI_CCR_ADMODE(xctn->addrmode) |
            OCTOSPI_CCR_ADSIZE(xctn->addrsize) |
            OCTOSPI_CCR_ABMODE(xctn->altbytesmode) |
            OCTOSPI_CCR_ABSIZE(xctn->altbytessize) |
            OCTOSPI_CCR_DMODE(xctn->datamode) |
            (xctn->isddr ? OCTOSPI_CCR_DDTR : 0) |
            (xctn->isddr ? OCTOSPI_CCR_ABDTR : 0) |
            (xctn->isddr ? OCTOSPI_CCR_ADDTR : 0);
  qspi_putreg(priv, regval, STM32_OCTOSPI_CCR_OFFSET);

  regval = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
  regval = (regval & ~(OCTOSPI_CR_FMODE_MASK)) | OCTOSPI_CR_FMODE(fctn);
  qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);

  regval = qspi_getreg(priv, STM32_OCTOSPI_TCR_OFFSET);
  regval = (regval & ~(OCTOSPI_TCR_DCYC_MASK)) |
            OCTOSPI_TCR_DCYC(xctn->dummycycles);
  qspi_putreg(priv, regval, STM32_OCTOSPI_TCR_OFFSET);

  /* Writing IR could trigger a transaction if:
   * ADMODE == 000 & (FMODE = 01 | DMODE = 000)
   */

  regval = xctn->instr;
  qspi_putreg(priv, regval, STM32_OCTOSPI_IR_OFFSET);

  /* If we have and need and address, set that now, too */

  if (CCR_ADMODE_NONE != xctn->addrmode && CR_FMODE_MEMMAP != fctn)
    {
      qspi_putreg(priv, xctn->addr, STM32_OCTOSPI_AR_OFFSET);
    }
}
...
static int qspi0_interrupt(int irq, void *context, void *arg)
{
  ...
  status  = qspi_getreg(&g_qspi0dev, STM32_OCTOSPI_SR_OFFSET);
  cr  = qspi_getreg(&g_qspi0dev, STM32_OCTOSPI_CR_OFFSET);

  /* Is it 'FIFO Threshold'? */

  if ((status & OCTOSPI_SR_FTF) && (cr & OCTOSPI_CR_FTIE))
    {
      volatile uint32_t *datareg =
        (volatile uint32_t *)(g_qspi0dev.base + STM32_OCTOSPI_DR_OFFSET);
      ...
      if (g_qspi0dev.xctn->function == CR_FMODE_INDWR)
        {
          ...
          while (((regval = qspi_getreg(
                 &g_qspi0dev, STM32_OCTOSPI_SR_OFFSET)) & OCTOSPI_SR_FTF) != 0)
            {
  ...
  if ((status & OCTOSPI_SR_TCF) && (cr & OCTOSPI_CR_TCIE))
    {
      /* Acknowledge interrupt */

      qspi_putreg(&g_qspi0dev, OCTOSPI_FCR_CTCF, STM32_OCTOSPI_FCR_OFFSET);

      /* Disable the QSPI FIFO Threshold, Transfer Error and Transfer
       * complete Interrupts
       */

      regval  = qspi_getreg(&g_qspi0dev, STM32_OCTOSPI_CR_OFFSET);
      regval &= ~(OCTOSPI_CR_TEIE | OCTOSPI_CR_TCIE | OCTOSPI_CR_FTIE);
      qspi_putreg(&g_qspi0dev, regval, STM32_OCTOSPI_CR_OFFSET);
      ...
      if (g_qspi0dev.xctn->function == CR_FMODE_INDRD)
        {
          ...
          while (((regval = qspi_getreg(
                 &g_qspi0dev, STM32_OCTOSPI_SR_OFFSET)) &
                 OCTOSPI_SR_FLEVEL_MASK) != 0)
            {
              if (g_qspi0dev.xctn->idxnow < g_qspi0dev.xctn->datasize)
                {
                  ((uint8_t *)g_qspi0dev.xctn->buffer)
  ...
  if ((status & OCTOSPI_SR_SMF) && (cr & OCTOSPI_CR_SMIE))
    {
      /* Acknowledge interrupt */

      qspi_putreg(&g_qspi0dev, OCTOSPI_FCR_CSMF, STM32_OCTOSPI_FCR_OFFSET);

      /* If 'automatic poll mode stop' is activated, we're done */

      if (cr & OCTOSPI_CR_APMS)
        {
          /* Disable the QSPI Transfer Error and Status Match Interrupts */

          regval  = qspi_getreg(&g_qspi0dev, STM32_OCTOSPI_CR_OFFSET);
          regval &= ~(OCTOSPI_CR_TEIE | OCTOSPI_CR_SMIE);
          qspi_putreg(&g_qspi0dev, regval, STM32_OCTOSPI_CR_OFFSET);
  ...
  if ((status & OCTOSPI_SR_TEF) && (cr & OCTOSPI_CR_TEIE))
    {
      /* Acknowledge interrupt */

      qspi_putreg(&g_qspi0dev, OCTOSPI_FCR_CTEF, STM32_OCTOSPI_FCR_OFFSET);

      /* Disable all the QSPI Interrupts */

      regval  = qspi_getreg(&g_qspi0dev, STM32_OCTOSPI_CR_OFFSET);
      regval &= ~(OCTOSPI_CR_TEIE | OCTOSPI_CR_TCIE | OCTOSPI_CR_FTIE |
                  OCTOSPI_CR_SMIE | OCTOSPI_CR_TOIE);
      qspi_putreg(&g_qspi0dev, regval, STM32_OCTOSPI_CR_OFFSET);
  ...
  if ((status & OCTOSPI_SR_TOF) && (cr & OCTOSPI_CR_TOIE))
    {
      /* Acknowledge interrupt */

      qspi_putreg(&g_qspi0dev, OCTOSPI_FCR_CTOF, STM32_OCTOSPI_FCR_OFFSET);
...
#elif defined(CONFIG_STM32L4_OCTOSPI_DMA)
/****************************************************************************
                           ...
                           struct qspi_xctnspec_s *xctn)
{
  ...
  stm32l4_dmasetup(priv->dmach, qspi_regaddr(priv, STM32_OCTOSPI_DR_OFFSET),
  ...
  qspi_waitstatusflags(priv, OCTOSPI_SR_TCF, 1);
  qspi_waitstatusflags(priv, OCTOSPI_SR_BUSY, 0);
  UP_MB();
                                 ...
                                 struct qspi_xctnspec_s *xctn)
{
  int ret = OK;
  volatile uint32_t *datareg =
    (volatile uint32_t *)(priv->base + STM32_OCTOSPI_DR_OFFSET);
  uint8_t *dest = (uint8_t *)xctn->buffer;
  uint32_t addrval;
  uint32_t regval;

  addrval = qspi_getreg(priv, STM32_OCTOSPI_AR_OFFSET);
  if (dest != NULL)
    {
      ...
      /* Ensure CR register specifies indirect read */

      regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
      regval &= ~OCTOSPI_CR_FMODE_MASK;
      regval |= OCTOSPI_CR_FMODE(CR_FMODE_INDRD);
      qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);

      /* Start the transfer by re-writing the address in AR or IR register */

      if (xctn->addrmode != CCR_ADMODE_NONE)
        {
          qspi_putreg(priv, addrval, STM32_OCTOSPI_AR_OFFSET);
        }
      else
        {
          qspi_putreg(priv, xctn->instr, STM32_OCTOSPI_IR_OFFSET);
        }

      if (qspi_getreg(priv, STM32_OCTOSPI_SR_OFFSET) & OCTOSPI_SR_TEF)
        {
          qspi_putreg(priv, OCTOSPI_FCR_CTEF, STM32_OCTOSPI_FCR_OFFSET);
          qspi_abort(priv);
          return -ENOTBLK;
        }
      ...
      while (remaining > 0)
        {
          ...
          qspi_waitstatusflags(priv, OCTOSPI_SR_FTF | OCTOSPI_SR_TCF, 1);
      ...
      if (ret == OK)
        {
          ...
          qspi_waitstatusflags(priv, OCTOSPI_SR_TCF, 1);
          qspi_putreg(priv, OCTOSPI_FCR_CTCF, STM32_OCTOSPI_FCR_OFFSET);
      ...
      while (remaining > 0)
        {
          ...
          qspi_waitstatusflags(priv, OCTOSPI_SR_FTF, 1);
...
static uint32_t qspi_setfrequency(struct qspi_dev_s *dev, uint32_t frequency)
{
   ...
   * STM32_OCTOSPI_CR_OFFSET register field holds prescaler - 1.
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_DCR2_OFFSET);
  regval &= ~(OCTOSPI_DCR2_PRESCALER_MASK);
  regval |= (prescaler - 1) << OCTOSPI_DCR2_PRESCALER_SHIFT;
  qspi_putreg(priv, regval, STM32_OCTOSPI_DCR2_OFFSET);
  ...
  if (mode != priv->mode)
    {
      ...
      regval  = qspi_getreg(priv, STM32_OCTOSPI_DCR1_OFFSET);
      regval &= ~(OCTOSPI_DCR1_CKMODE);
      ...
      qspi_putreg(priv, regval, STM32_OCTOSPI_DCR1_OFFSET);
      spiinfo("DCR1=%08" PRIx32 "\n", regval);
...
static void qspi_setbits(struct qspi_dev_s *dev, int nbits)
{
  /* Not meaningful for the STM32L4+x6 */
                        ...
                        struct qspi_cmdinfo_s *cmdinfo)
{
  ...
  qspi_putreg(priv,
              OCTOSPI_FCR_CTEF | OCTOSPI_FCR_CTCF | OCTOSPI_FCR_CSMF | OCTOSPI_FCR_CTOF,
              STM32_OCTOSPI_FCR_OFFSET);
  ...
  if (QSPICMD_ISDATA(cmdinfo->flags))
      ...
      if (QSPICMD_ISWRITE(cmdinfo->flags))
        {
          ...
          regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
          regval |= (OCTOSPI_CR_TEIE | OCTOSPI_CR_FTIE | OCTOSPI_CR_TCIE);
          qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);
        }
      else
        {
  ...
  else
    {
      ...
      regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
      regval |= (OCTOSPI_CR_TEIE | OCTOSPI_CR_TCIE);
      qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);
                       ...
                       struct qspi_meminfo_s *meminfo)
{
  ...
  if (QSPIMEM_ISWRITE(meminfo->flags))
    {
      ...
      /* Set up the Communications Configuration Register as per meminfo */
  ...
  if (priv->candma &&
      meminfo->buflen > CONFIG_STM32L4_OCTOSPI_DMATHRESHOLD &&
...
static int qspi_hw_initialize(struct stm32_qspidev_s *priv)
{
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
  regval &= ~(OCTOSPI_CR_TEIE | OCTOSPI_CR_TCIE | OCTOSPI_CR_FTIE | OCTOSPI_CR_SMIE |
              OCTOSPI_CR_TOIE | OCTOSPI_CR_MSEL | OCTOSPI_CR_DMM);

#if defined(CONFIG_STM32L4_OCTOSPI_MODE_BANK2)
  regval |= OCTOSPI_CR_MSEL;
#endif

#if defined(CONFIG_STM32L4_OCTOSPI_MODE_DUAL)
  regval |= OCTOSPI_CR_DMM;
#endif

  /* Configure QSPI FIFO Threshold */

  regval &= ~(OCTOSPI_CR_FTHRES_MASK);
  regval |= ((CONFIG_STM32L4_OCTOSPI_FIFO_THRESHOLD - 1) <<
    OCTOSPI_CR_FTHRES_SHIFT);
  qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_DCR2_OFFSET);
  regval &= ~(OCTOSPI_DCR2_PRESCALER_MASK);
  regval |= (0x01 << OCTOSPI_DCR2_PRESCALER_SHIFT);
  qspi_putreg(priv, regval, STM32_OCTOSPI_DCR2_OFFSET);

  regval  = qspi_getreg(priv, STM32_OCTOSPI_TCR_OFFSET);
  regval &= ~(OCTOSPI_TCR_SSHIFT);
  qspi_putreg(priv, regval, STM32_OCTOSPI_TCR_OFFSET);
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_DCR1_OFFSET);
  regval &= ~(OCTOSPI_DCR1_CKMODE |
              OCTOSPI_DCR1_CSHT_MASK |
              OCTOSPI_DCR1_DEVSIZE_MASK);

  regval |= ((CONFIG_STM32L4_OCTOSPI_CSHT - 1) << OCTOSPI_DCR1_CSHT_SHIFT);
  if (0 != CONFIG_STM32L4_QSPI_FLASH_SIZE)
    {
      ...
      regval |= ((nlog2size - 1) << OCTOSPI_DCR1_DEVSIZE_SHIFT);
    }
  ...
  if (intf == 0)
    {
      ...
      /* Enable clocking to the OCTOSPIM IO Manager (AHB2) */

      regval = getreg32(STM32L4_RCC_AHB2ENR);
      regval |= RCC_AHB2ENR_OSPIMEN;
      putreg32(regval, STM32L4_RCC_AHB2ENR);

      /* Enable clocking to the OCTOSPI peripheral (AHB3) */

      regval = getreg32(STM32L4_RCC_AHB3ENR);
      regval |= RCC_AHB3ENR_OSPI1EN;
      putreg32(regval, STM32L4_RCC_AHB3ENR);

      /* Reset the OCTOSPI peripheral */

      regval = getreg32(STM32L4_RCC_AHB3RSTR);
      regval |= RCC_AHB3RSTR_OSPI1RST;
                                     ...
                                     uint32_t lpto)
{
  ...
  if (lpto > 0)
    {
      ...
      qspi_putreg(priv, lpto, STM32_OCTOSPI_LPTR_OFFSET);
      ...
      regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
      regval |= (OCTOSPI_CR_TCEN | OCTOSPI_CR_TOIE);
      qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);
#endif
    }
  else
  ...
  qspi_ccrconfig(priv, &xctn, CR_FMODE_MEMMAP);
  priv->memmap = true;
